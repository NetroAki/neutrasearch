...
- Compaction runs only after a threshold is crossed, at low I/O/CPU priority, and is interruptible. It does not run merely because the service is idle.
...
- Linux can run `neutrasearch serve --index INDEX.nsx --watch MOUNT`. It uses a filesystem fanotify mark with file handles and names, performs only direct event-target metadata reads, batches/coalesces changes, and stops with an explicit stale error on queue overflow. It requires `CAP_SYS_ADMIN`, `CAP_DAC_READ_SEARCH`, kernel/filesystem file-handle support, and an explicit source ID for multi-source bases. The race-free initial-scan-to-watch handoff is not complete yet, so this is not a full freshness claim.
- Windows USN Journal checkpoints and macOS FSEvents/Spotlight state remain to be wired into the same update service.
- Automatic replacement-base compaction after the delta threshold is the next durability step; the service currently reports `needs_compaction` rather than silently rewriting the base.
