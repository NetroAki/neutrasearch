...
if ($null -ne $existing) {
    ...
    if ($null -eq $serviceConfig) {
        throw "Could not read $serviceName configuration"
    }
    ...
    if ($change.ReturnValue -ne 0) {
        throw "Could not update $serviceName service (Win32_Service.Change return $($change.ReturnValue))"
    }
    & "$env:SystemRoot\System32\sc.exe" description $serviceName 'Local-only native NTFS metadata scanner for Neutrasearch.' | Out-Null
    if ($LASTEXITCODE -ne 0) {
        throw "Could not update $serviceName description (sc.exe exit $LASTEXITCODE)"
    }
} else {
    New-Service `
        -Name $serviceName `
